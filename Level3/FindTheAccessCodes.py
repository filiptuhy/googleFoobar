...
#import itertools
#def answer(lst):
#	allp2 = [x for x in itertools.combinations(lst,3) if x[2] % x[1] == 0 and x[1] % x[0]==0]
#	return len(allp2)
#.................
 
# RE READ THE TASK I<J<K so I should not sort
"""
def answer(lst):
	maxLst = lst
	count =0
	for i in range(len(lst)):
		for j in range(1,len(lst)):
				for k in range(2,len(lst)):
					if maxLst[i] !=maxLst[j] and maxLst[j]!=maxLst[k]:
						if maxLst[i] % maxLst[j] == 0 and maxLst[j] % maxLst[k] == 0:
							count +=1
	return count
"""	
